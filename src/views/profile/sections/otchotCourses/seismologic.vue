<template>
    <div class="otch_card" v-for="(report, index) in reports" :key="index">
      <div class="bg__mask"></div>
      <div class="body">
        <strong>{{ report.name }}</strong>
        <p class="name">{{report.labName}} {{ report.labNumber }}</p>
        <div class="files">
          <a href="#!">
            <svg
              width="40"
  ...
  export default {
    data() {
      return {
        ...
        course : this.$t("seismologicName")
      }
    },
    props: {
      title: {
        type: String,
        default: "Магниторазведка",
      },
      name: {
        type: String,
        default: "№1",
    ...
    computed : {
      reports () {
        return [
          {
            name : this.$t("seismologicName"),
            labLink : "",
            labName : this.$t("labWork"),
            labNumber : "№1"
          },
          {
            name : this.$t("seismologicName"),
  ...
  .otch_card {
    ...
    margin-bottom: 20px;
    margin: 0 17px 20px;
    border-left-color: rgb(173, 221, 221);
    .bg__mask {
      display: flex;
      position: absolute;
      width: 100%;
